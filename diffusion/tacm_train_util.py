    ...
    def run_loop(self):
        while (
            ...
            for i, sample in enumerate(self.data):
                ...
                # encode the video input
                #z = self.first_stage_model.content_encoder(batch) #torch.Size([1, 256, 16, 8, 8])
                #z = self.first_stage_model.pre_vq_conv(z)
                #vq_output = self.first_stage_model.codebook(z)
                #embeddings = vq_output['embeddings']*0.1 # torch.Size([1, 256, 16, 8, 8])
                #embeddings = rearrange(embeddings, "b c t h w -> (b t) c h w")

                #indices = vq_output['encodings'] * 0.0001 #torch.Size([1, 16, 8, 8])
                #indices = indices.unsqueeze(1)
                #indices = vq_output['encodings'].float().unsqueeze(1) #torch.Size([1, 1, 16, 8, 8])
