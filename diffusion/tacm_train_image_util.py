...
class TrainLoop():
    def __init__(
        ...
        *,
        ...
        self.model = CLIPModel.from_pretrained("openai/clip-vit-large-patch14").to(dist_util.dev())
        ...
        while (
            ...
            for i, sample in enumerate(self.data):
                ...
                for i in range(image.shape[0]):
                    image_i = transformers.image_transforms.to_pil_image(image[i])
                    image_input = self.processor(images=image_i, return_tensors="pt", padding=True).to(dist_util.dev())
                    with th.no_grad():
                        image_features = self.model.get_image_features(image_input.pixel_values)
